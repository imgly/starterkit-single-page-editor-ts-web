...
│   └── imgly/
│       ├── index.ts          # Editor initialization with page-select component
│       └── config/
│           ├── plugin.ts         # Main plugin orchestration
│           ├── actions.ts        # Load, Save, Export actions
│           ├── features.ts       # Feature toggles
│           ├── settings.ts       # Engine behavior (singlePageModeEnabled)
│           ├── i18n.ts           # Internationalization
│           └── ui/               # UI layout configuration
├── public/                   # Static assets (includes template archive)
├── package.json
└── vite.config.ts
```
